...
class RuleLoaderTests(unittest.TestCase):
    ...
    def test_scope_reports_existing_draft_rule_corpus_and_missing_chapter_20(self) -> None:
        ...
        self.assertEqual(scope.verified_rules, 0)
        self.assertEqual(scope.draft_rules, scope.total_rules)
    ...
    def test_legacy_chapter_mismatches_are_reported_as_warnings(self) -> None:
        ...
        self.assertTrue(any("citation" in warning for warning in scope.warnings))
        self.assertTrue(any("normalized legacy rule id" in warning for warning in scope.warnings))
